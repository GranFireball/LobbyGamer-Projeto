...
export default function PlayerCard({player, status}){
  ...
  const {mutate: sendFriendRequest} = useMutation({
    mutationFn: async () => {
      const response = await fetch("http://localhost:3001/players/" + sistemaPlayer.player.id + "/addfriendrequest/" + player.id, {
        method: "PUT",
        headers: {
  ...
  const {mutate: addFriend} = useMutation({
    mutationFn: async () => {
      const response = await fetch("http://localhost:3001/players/" + sistemaPlayer.player.id + "/addfriend/" + player.id, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json"
        }
  ...
  const {mutate: removeFriendRequest} = useMutation({
    mutationFn: async () => {
      const response = await fetch("http://localhost:3001/players/" + sistemaPlayer.player.id + "/removefriendrequest/" + player.id, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json"
        }
